...
function randomImg(g) {
    if (g == undefined) g = rn;else g = rng(g);
...
function httpGet(url, cont) {
    ...
    xhReq.onload = function (e) {
        console.log(e);cont(e.currentTarget.responseText, e);
    };
    xhReq.send(null);
}
...
function ipfsGetImg(hash, format, cont) {
    if (ipfsCacheArrayBuffer[hash] == undefined) {
        ...
        if (ipfsCacheArrayBuffer[hash].content != undefined) cont("data:image/" + format + ";base64," + ipfsCacheArrayBuffer[hash].content);else ipfsCacheArrayBuffer[hash].callBacks.push(function (data) {
            return cont("data:image/" + format + ";base64," + data);
        });
    }
}
...
function ipfsGet(hash, cont) {
    if (ipfsCache[hash] == undefined) {
        ...
        httpGet(baseApiUrl + "cat?arg=" + hash, function (data) {
        ...
        if (ipfsCache[hash].content == undefined) ipfsCache[hash].callBacks.push(cont);else cont(ipfsCache[hash].content);
    }
}
...
function pointIpnsTo(hash, cont) {
    ...
    httpGet(baseApiUrl + "name/publish?arg=" + hash, function (e) {
        setStatus("Ready");cont();
    });
}
...
function saveRoom(userChatConfig, name, json, cont) {
    if (json == undefined || json == null) {
        console.log("ERROR SAVING UNDEFINED ROOM", json, name, cont);return;
    }
...
function addChatMessage(userChatConfig, roomName, content, cont) {
    ipfsGet(userChatConfig[roomName], function (r) {
        safeParseCont(r, emptyFunction, function (room) {
            ...
            if (room.userIcon) message.icon = room.userIcon;

            room.hotMessages.push(message);
            sortMessages(room.hotMessages);
            getChatLog(room, function (log) {
                return setChatContent(userChatConfig, log);
            });
...
function init() {
    if (document.location.href.indexOf("api=") != -1) baseApiUrl = document.location.href.split("api=")[1];

    var failFunction = function failFunction() {
        setStatus("You need to press the init button!!!");generateUI(appState.userChatConfig, appState.currentRoom);
    };
                ...
                ipfsGet(appState.userPointedHash, function (r) {
                    safeParseCont(r, failFunction, function (config) {
                        ...
                        var updateTimer = setInterval(function () {
                            update(appState.userChatConfig, appState.currentRoom);
                        }, 1000 * 60 * 1);
...
function setStatus(s) {
    ...
    if (window["statusDiv"]) window["statusDiv"].innerHTML = s;else console.log("WARNING STATUS DIV NOT PRESENT");
}
...
function update(userChatConfig, roomName) {
    withRoom(userChatConfig[roomName], function (room) {
                            ...
                            getChatLog(room, function (log) {
                                ...
                                saveRoom(userChatConfig, roomName, room, function () {
                                    if (n >= room.others.length - 1) {
                                        ...
                                        archiveMessages(userChatConfig, roomName, function () {
                                            return refreshChatcontent(userChatConfig);
                                        });
                                    } else updateOtherUsers(n + 1);
                                });
                            });
                        });
                    });
        ...
        if (room.others.length != 0) updateOtherUsers(0);else setStatus("Ready");
    });
}

function refreshChatcontent(userChatConfig) {
    withRoom(appState.userChatConfig[appState.currentRoom], function (room) {
        getChatLog(room, function (log) {
            return setChatContent(userChatConfig, log);
        });
    });
            ...
            if (room.hotMessages.length != 0) {
                ...
                if (lm.timeStamp < timeStamp() - room.hotMessageAgeLimit) {
                    ...
                    ipfsAdd(JSON.stringify(message), function (mHash) {
                        room.archive = mHash;
                        saveRoom(userChatConfig, roomName, room, function () {
                            archiveMessages(userChatConfig, roomName, cont);
                        });
                    });
                } else cont();
            }
    ...
    ipfsGet(userChatConfig[roomName], function (r) {
        safeParseCont(r, error("ERROR PARSING ROOM DATA"), function (room) {
            ...
            if (room.logo != undefined) {
                ipfsGetImg(room.logo, "jpg", function (data) {
                    logoDiv.style.backgroundImage = "url(" + data + ")";
                });
            }
            if (room.background != undefined) {
                ipfsGetImg(room.background, "jpg", function (data) {
            ...
            onEnter(addChatRoomInput, function () {
                ipfsAdd(JSON.stringify(defaultRoomConfig()), function (hash) {
                    ...
                    saveChatconfig(userChatConfig, function () {
                        return generateUI(userChatConfig, appState.currentRoom);
                    });
                });
            });
            ...
            for (var i in userChatConfig) {
                ...
                b.onclick = (function (i) {
                    return function () // extra function and strange calling to fix javascript scopping
                    {
...
function setChatContent(userChatConfig, chatLog) {
    ...
    for (var i = chatLog.length - 1; i >= 0; i--) {
        if (chatLog[i] != null && chatLog[i] != undefined) {
            ...
            if (chatLog[i].icon) {
                ipfsGetImg(chatLog[i].icon, "jpg", (function (img, imgSide) {
        ...
        } else console.log("NULL OR UNDEFINED MESSAGE IN SET CHAT CONTENT", chatLog);
    }
}
...
function getChatLogArchive(startHash, num, cont) {
    if (!startHash || num == 0) cont([]);else {
        var loop = function loop(hash, list, num, funct, cont) {
            ipfsGet(hash, function (r) {
                var message = JSON.parse(r);

                list.push(message);

                if (!message.parent || num == 0) cont(list);else funct(message.parent, list, num - 1, funct, cont);
            });
        };
